...
const SignIn = () => {
    ...
    const watchField = (e) => {
        return fields[e] === "" ? (<p className="text-red-400 text-sm mt-2">Sorry but your {e} is required</p>) : '';
    }
            ...
            <div className="mx-auto my-10 min-h-[200px] rounded max-w-[400px]">
                <form onSubmit={submitForm} className="block p-2 relative w-100">
                    <fieldset className="form-group relative block w-100 mb-5">
                        <label htmlFor="name" className="text-sm">Whatsapp Name</label>

                        <input autoComplete="off" onInput={(e) => setFields({...fields, name: e.target.value})}
                               value={fields.name.trimStart()} required type="text" id="name" placeholder="Whatsapp name"
                    ...
                    <fieldset className="form-group relative block w-100 mb-4">
                        ...
                        <input type="password" onInput={(e) => setFields({...fields, password: e.target.value})}
                               value={fields.password} required autoComplete={"off"} id="password"
                               className="form-control mt-2 block border bg-transparent outline-0 border-gray-500 h-[40px] rounded px-3 w-full"/>
